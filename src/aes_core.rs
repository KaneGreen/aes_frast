...
// 192bit key schedule
macro_rules! setkey_192_function {
    ($origin:ident, $keys:ident) => {{
        for i in 0..6 {
            $keys[i] = four_u8_to_u32!($origin[4 * i], $origin[4 * i + 1], $origin[4 * i + 2], $origin[4 * i + 3]);
        }
...
/// 256bit | 32 | 60
/// 
/// This function is an alternative to [`setkey_enc_k128`], [`setkey_enc_k192`] and [`setkey_enc_k256`] functions. Which one to use is up to you.
/// # Examples
/// Please refer to [`setkey_enc_k128`], [`setkey_enc_k192`] and [`setkey_enc_k256`] functions, they are very similar but differ in the outputs.
/// 
/// [`setkey_enc_k128`]: ../aes_core/fn.setkey_enc_k128.html
/// [`setkey_enc_k192`]: ../aes_core/fn.setkey_enc_k192.html
/// [`setkey_enc_k256`]: ../aes_core/fn.setkey_enc_k256.html
pub fn setkey_enc_auto(origin: &[u8], keys: &mut [u32]) {
    match origin.len() {
        16 => setkey_128_function!(origin, keys),
        24 => setkey_192_function!(origin, keys),
        32 => setkey_256_function!(origin, keys),
...
/// 
/// setkey_enc_k128(&origin, &mut output);
/// 
/// let expected: [u32; 44] = [
...
pub fn setkey_enc_k192(origin: &[u8], keys: &mut [u32]) {
    setkey_192_function!(origin, keys);
}
...
/// 256bit | 32 | 60
/// 
/// This function is an alternative to [`setkey_dec_k128`], [`setkey_dec_k192`] and [`setkey_dec_k256`] functions. Which one to use is up to you.
/// # Examples
/// Please refer to [`setkey_dec_k128`], [`setkey_dec_k192`] and [`setkey_dec_k256`] functions, they are very similar but differ in the outputs.
/// 
...
pub fn setkey_dec_auto(origin: &[u8], keys: &mut [u32]) {
    match origin.len() {
        16 => {
            setkey_128_function!(origin, keys);
...
/// let mut output: [u8; 16] = [0; 16];
/// 
/// setkey_enc_k192(&origin_key, &mut scheduled_keys);
/// block_enc_k192(&input, &mut output, &scheduled_keys);
/// 
/// let expected: [u8; 16] = [
...
/// }
/// ```
pub fn block_enc_k192(input: &[u8], output:&mut [u8], keys: &[u32]) {
    encryption_function!(input, output, keys, 6, 52);
}
...
/// let mut output: [u8; 16] = [0; 16];
/// 
/// setkey_dec_k128(&origin_key, &mut scheduled_keys);
/// block_dec_k128(&input, &mut output, &scheduled_keys);
/// 
/// let expected: [u8; 16] = [
///     0x32, 0x43, 0xF6, 0xA8, 0x88, 0x5A, 0x30, 0x8D,
///     0x31, 0x31, 0x98, 0xA2, 0xE0, 0x37, 0x07, 0x34
...
pub fn block_dec_k192(input: &[u8], output:&mut [u8], keys: &[u32]) {
    decryption_function!(input, output, keys, 6, 52);
}
...
mod tests {
    use super::{*};
    #[test]
    fn setkey_dec_k128_works() {
        let origin: [u8; 16] = [
            0x2B, 0x7E, 0x15, 0x16, 0x28, 0xAE, 0xD2, 0xA6,
            0xAB, 0xF7, 0x15, 0x88, 0x09, 0xCF, 0x4F, 0x3C
        ];
        let mut output: [u32; 44] = [0; 44];
        setkey_dec_k128(&origin, &mut output);
        let expected: [u32; 44] = [
            0x2B7E1516, 0x28AED2A6, 0xABF71588, 0x09CF4F3C,
    ...
    fn setkey_dec_k192_works() {
        ...
        let mut output: [u32; 52] = [0; 52];
        setkey_dec_k192(&origin, &mut output);
        let expected: [u32; 52] = [
            0x8E73B0F7, 0xDA0E6452, 0xC810F32B, 0x809079E5,
    ...
    fn setkey_dec_k256_works() {
        ...
        let mut output: [u32; 60] = [0; 60];
        setkey_dec_k256(&origin, &mut output);
        let expected: [u32; 60] = [
            0x603DEB10, 0x15CA71BE, 0x2B73AEF0, 0x857D7781,
    ...
    #[test]
    fn setkey_enc_auto_works() {
        let origin128: [u8; 16] = [
            0x2B, 0x7E, 0x15, 0x16, 0x28, 0xAE, 0xD2, 0xA6,
            0xAB, 0xF7, 0x15, 0x88, 0x09, 0xCF, 0x4F, 0x3C
        ];
        let mut output128a: [u32; 44] = [0; 44];
        let mut output128b: [u32; 44] = [0; 44];
        setkey_enc_auto(&origin128, &mut output128a);
        setkey_enc_k128(&origin128, &mut output128b);
        for i in 0..44 {
            assert_eq!(output128a[i], output128b[i]);
        }
        let origin192: [u8; 24] = [
            0x8E, 0x73, 0xB0, 0xF7, 0xDA, 0x0E, 0x64, 0x52,
            0xC8, 0x10, 0xF3, 0x2B, 0x80, 0x90, 0x79, 0xE5,
        ...
        let mut output192a: [u32; 52] = [0; 52];
        let mut output192b: [u32; 52] = [0; 52];
        setkey_enc_auto(&origin192, &mut output192a);
        setkey_enc_k192(&origin192, &mut output192b);
        for i in 0..52 {
            assert_eq!(output192a[i], output192b[i]);
        }
        let origin256: [u8; 32] = [
            0x60, 0x3D, 0xEB, 0x10, 0x15, 0xCA, 0x71, 0xBE,
            0x2B, 0x73, 0xAE, 0xF0, 0x85, 0x7D, 0x77, 0x81,
        ...
        let mut output256a: [u32; 60] = [0; 60];
        let mut output256b: [u32; 60] = [0; 60];
        setkey_enc_auto(&origin256, &mut output256a);
        setkey_enc_k256(&origin256, &mut output256b);
        for i in 0..60 {
            assert_eq!(output256a[i], output256b[i]);
        }
    }
    #[test]
    fn setkey_dec_auto_works() {
        let origin128: [u8; 16] = [
            0x2B, 0x7E, 0x15, 0x16, 0x28, 0xAE, 0xD2, 0xA6,
            0xAB, 0xF7, 0x15, 0x88, 0x09, 0xCF, 0x4F, 0x3C
        ];
        let mut output128a: [u32; 44] = [0; 44];
        let mut output128b: [u32; 44] = [0; 44];
        setkey_dec_auto(&origin128, &mut output128a);
        setkey_dec_k128(&origin128, &mut output128b);
        for i in 0..44 {
            assert_eq!(output128a[i], output128b[i]);
        }
        let origin192: [u8; 24] = [
        ...
        let mut output192a: [u32; 52] = [0; 52];
        let mut output192b: [u32; 52] = [0; 52];
        setkey_dec_auto(&origin192, &mut output192a);
        setkey_dec_k192(&origin192, &mut output192b);
        for i in 0..52 {
            assert_eq!(output192a[i], output192b[i]);
        }
        let origin256: [u8; 32] = [
        ...
        let mut output256a: [u32; 60] = [0; 60];
        let mut output256b: [u32; 60] = [0; 60];
        setkey_dec_auto(&origin256, &mut output256a);
        setkey_dec_k256(&origin256, &mut output256b);
        for i in 0..60 {
            assert_eq!(output256a[i], output256b[i]);
        }
    }
